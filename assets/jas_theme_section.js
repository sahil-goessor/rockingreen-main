...
(function ($) {
  ...
  geckoShopify = (function () {
    var geckoTheme = {
      ...
      ajaxLinks:
        ".widget_product_categories a, .jas_ajaxFilter a, .jas-pagination-ajax a, .nav_filters a, .widget_product_tag_cloud a",
    };
    return {
      init: function () {
        this.initCarousel();
      ...
      shopMasonry: function () {
        ...
        $(window).resize(function () {
          $(".categories-masonry").each(function (index) {
            var $el = $(this);
            var colWidth = $el.hasClass("categories-style-masonry")
              ? ".jas-item-category"
      ...
      productsLoadMore: function () {
        ...
        $(document)
          .off("click", ".btn-products-load-more")
          .on("click", ".btn-products-load-more", function (e) {
            e.preventDefault();
            if (process) return;
            process = true;
            var $this = $(this),
            ...
            loadProducts(holder, jas_ajaxurl, $this, function (data) {
              ...
              holder.imagesLoaded().progress(function () {
                geckoShopify.clickOnScrollButton(
                  geckoTheme.shopLoadMoreBtn,
                  true
                );
              });
        ...
        var loadProducts = function (holder, ajaxurl, btn, callback) {
          ...
          $.ajax({
            ...
            complete: function () {
              ...
              if (
                gecko_settings.show_multiple_currencies &&
                $cookieCurrency !== null
              ) {
                Currency.convertAll(
                  shopCurrency,
                  $cookieCurrency,
                  ".jas-products-holder span.money"
                );
              }
              //product review
      ...
      jas_currency: function () {
        if (
          gecko_settings.show_multiple_currencies &&
          gl_Currency.cookie.read() !== null
        ) {
          Currency.convertAll(
            shopCurrency,
            gl_Currency.cookie.read(),
            'form[action="/cart/add"] span.money'
          );
        }
      },
      ...
      searchDropdown: function () {
        ...
        body.on("click", ".gecko-search-dropdown", function (e) {
          e.preventDefault();
          if (isOpened()) {
            closeWidget();
          } else {
        ...
        body.on(
          "click",
          ".gecko-close-search, #jas-header, #jas-content",
          function (event) {
            if (
              !$(event.target).is(".gecko-close-search") &&
              $(event.target).closest(".wrapper-search-dropdown").length
            )
              return;
        ...
        search_form.each(function () {
          ...
          input.attr("autocomplete", "off").bind("keyup", function () {
            ...
            if (term.trim() == "") {
              ...
              currentAjaxRequest = jQuery
                .get(searchURL + "&view=json", function (data) {
      ...
      initfooterMenu: function () {
        if (
          $(window).width() >= 768 &&
          !$("#jas_footer").hasClass("footer__collapsed")
        )
          return;
                  ...
                  open: function () {
                    ...
                    if (
                      ...
                      Currency.convertAll(
                        shopCurrency,
                        $cookieCurrency,
                        ".product-quick-view span.money"
                      );
                    }
                    if (
                      gecko_settings.productreviews &&
                      $(".spr-badge").length > 0
                    ) {
                      return (
                        window.SPR.registerCallbacks(),
      ...
      catelog_mode: function () {
        ...
        var html = [
          ...
          '<a href="#" class="cate-close-popup button view-cart">' +
            gecko_settings.catelog_btn +
            "</a>",
          "</div>",
        ].join("");
        ...
        $.magnificPopup.open({
          callbacks: {
            beforeOpen: function () {
              this.st.mainClass =
                geckoTheme.popupEffect + " cart-popup-wrapper";
            },
          },
          items: {
            src:
              '<div id="login-wishlist" class="jas_login-wishlist white-popup popup-catelog_mode popup-added_to_cart">' +
              html +
              "</div>",
            type: "inline",
          },
      ...
      loginWishlist: function () {
        ...
        body.on("click", ".nitro_wishlist_login", function (event) {
          ...
          var html = [
            ...
            '<a href="#" class="btn-style-link close-popup">' +
              gecko_settings.continue_shopping +
              "</a>",
            '<a href="/account/login" class="button view-cart">' +
              gecko_settings.login +
              "</a>",
            "</div>",
          ].join("");
          // Append new quantity selector then remove original
          $.magnificPopup.open({
            ...
            items: {
              src:
                '<div id="login-wishlist" class="jas_login-wishlist white-popup add-to-cart-popup popup-added_to_cart">' +
                html +
                "</div>",
              type: "inline",
            },
              ...
              if (data.status == "success" && status == "success") {
                ...
                if (rowCount < 1) {
                  ...
                  $(".wishlist_table tbody").append(
                    '<tr><td colspan="6" class="wishlist-empty tc">' +
                      gecko_settings.nowishlist +
                      "</td>"
                  );
                }
              } else {
            ...
            success: function (cart) {
              $.get("/cart?view=json", function (data, status) {
                ...
                $(".gecko-cart-subtotal >span").html(
                  Shopify.formatMoney(subtotal, gecko_settings.moneyFormat)
                );
                $(".cartCount").html(
                  $(".widget_shopping_cart_body").data("count")
                );
                geckoShopify.favicon_counter(
        ...
        if (gecko_settings.add_to_cart_action == "popup") {
          var html = [
            ...
            '<a href="#" class="btn btn-style-link close-popup">' +
              gecko_settings.continue_shopping +
              "</a>",
            '<a href="/cart" class="button view-cart">' +
              gecko_settings.view_cart +
              "</a>",
            "</div>",
          ].join("");
          $.magnificPopup.open({
          ...
          if ($(".jas-sidebar-cart").length > 0) {
            ...
            if (
              ...
              $("#jas-header").hasClass("header-sticky") == false
            ) {
              $("html, body").animate(
                {
                  scrollTop: 0,
          ...
          el.each(function (i, val) {
            ...
            if (_option !== undefined) {
              var _speed = _option.speed ? _option.speed : 700,
                ...
                _navp = _option.navigationPosition
                  ? _option.navigationPosition
                  : "left";
        ...
        $("body").on("click", ".sp-col-switch a", function (e) {
          ...
          if ($btnload.length > 0) {
            var href = $btnload.attr("href"),
              text = href
                .replace("inajax15", "inajax" + _col)
                .replace("inajax12", "inajax" + _col)
                .replace("inajax6", "inajax" + _col)
                .replace("inajax4", "inajax" + _col)
                .replace("inajax3", "inajax" + _col)
                .replace("inajax2", "inajax" + _col);
          ...
          if ($products.hasClass("metro")) {
            ...
            if (_col == "3" || _col == "2") {
              _product_small
                .removeClass(
                  "jas-col-md-2 jas-col-md-3 jas-col-md-4 jas-col-md-6 jas-col-md-12 jas-col-md-15"
                )
          ...
          _sizer
            .removeClass("size-2 size-3 size-4 size-6 size-12 size-15")
            .addClass("size-" + _col);
      ...
      initCountdown: function () {
        $(".jas-countdown").each(function () {
          $(this)
            .countdown($(this).data("time"), { elapse: true })
                ...
                $(this).html(
                  event.strftime(
                    "" +
                      ...
                      gecko_settings.countdown_days +
                      "</span></div>" +
                      '<div class="pr"><span class="db cw fs__16 mt__10">%H</span><span class="db">' +
                      gecko_settings.countdown_hours +
                      "</span></div>" +
                      '<div class="pr"><span class="db cw fs__16 mt__10">%M</span><span class="db">' +
                      gecko_settings.countdown_mins +
                      "</span></div>" +
                      '<div class="pr"><span class="db cw fs__16 mt__10">%S</span><span class="db">' +
                      gecko_settings.countdown_sec +
                      "</span></div>"
                  )
                );
              }
            ...
            if (days > 0) {
              function get1dayFromNow() {
                var clac_minutes = Math.floor(seconds / 60) % 60,
                  clac_date =
                    (hours + (minutes + clac_minutes) / 60) * 60 * 60 * 1000;
                return new Date(
                  new Date().valueOf() + days * 24 * 60 * 60 * 1000 + clac_date
                );
              }
              $(this)
                .countdown(get1dayFromNow())
                .on("update.countdown", function (event) {
                  var $this = $(this);
                  $(this).html(
                    event.strftime(
      ...
      ajaxFilters: function () {
        ...
        var scrollToTop = function () {
          ...
          var $scrollTo = $(".main-page-wrapper"),
            scrollTo =
              $scrollTo.offset().top - gecko_settings.ajax_scroll_offset;
      ...
      productImages: function () {
        ...
        var getProductItems = function () {
          ...
          $mainImages.find(".jas_img_ptw a img").each(function () {
            ...
            items.push({
              ...
              h: height,
              title:
                gecko_settings.product_images_captions == "yes"
                  ? caption
                  : false,
            });
          });
        ...
        if ($productGallery.hasClass("image-action-zoom")) {
          ...
          zoom_target.each(function () {
            var $this = $(this);
            if (
              $this.attr("data-large_image_width") >
              $(".shopify-product-gallery__image").width()
            ) {
              $this.trigger("zoom.destroy");
              var zoom_parent = $this.closest(
          ...
          window.jasParallaxSkroll && window.jasParallaxSkroll.destroy(),
            ...
            $("[data-jas-parallax]").each(function () {
              var skrollrSpeed,
                ...
                $parallaxElement,
                parallaxImage,
                srcset_paralax = $(this).attr("data-bgset"),
                sizes_paralax = $(this).attr("data-sizes"),
                fit_paralax = $(this).attr("data-parent-fit");
              (callSkrollInit = !0),
                "on" === $(this).data("jasParallaxOFade") &&
                  $(this)
                    .children()
                    .attr("data-5p-top-bottom", "opacity:0;")
                    .attr("data-30p-top-bottom", "opacity:1;"),
                (skrollrSize = 100 * $(this).data("jasParallax")),
                ($parallaxElement = $("<div />")
                  .addClass("jas_parallax-inner")
                  .appendTo($(this))),
                (parallaxImage = $(this).data("jas-parallax-image")),
                $parallaxElement.addClass("lazyload"),
                $parallaxElement.css(
                  "background-image",
                  "url(" + parallaxImage + ")"
                ),
                $parallaxElement.attr({
            ...
            !(!callSkrollInit || !window.skrollr) &&
              ...
              (window.jasParallaxSkroll = skrollr.init(jasSkrollrOptions)),
            ...
            complete: function () {
              ...
              if (
                ...
                Currency.convertAll(
                  shopCurrency,
                  $cookieCurrency,
                  ".widget_recently_viewed_products span.money"
                );
              }
            },
          });
      ...
      promoPopup: function () {
        ...
        if (
          ...
          gecko_settings.enable_popup != "yes" ||
          $(".gecko-promo-popup ").length == 0 ||
          (gecko_settings.promo_popup_hide_mobile == "yes" &&
            $(window).width() < 768)
        )
          return;
        ...
        var showPopup = function () {
          $.magnificPopup.open({
            ...
            callbacks: {
              beforeOpen: function () {
                this.st.mainClass =
                  gecko_settings.popupEffect + " promo-popup-wrapper";
              },
              open: function () {
              ...
              close: function () {
                gecko_cookie("gecko_popup_" + promo_version, "shown", {
                  expires: 7,
                  path: "/",
                });
              },
      ...
      cookiesPopup: function () {
        ...
        if (gecko_cookie("gecko_cookies_law_" + cookies_version) == "accepted")
          return;
        ...
        var acceptCookies = function () {
          ...
          gecko_cookie("gecko_cookies_law_" + cookies_version, "accepted", {
            expires: 60,
            path: "/",
          });
        };
      ...
      shopifyWrappTable: function () {
        var sopTable = $(
          ".shop_table:not(.shop_table_responsive):not(.jas-shopify-table)"
        ).wrap("<div class='jas-table-responsive'></div>");
      },
        ...
        $(".input-dropdown-inner_pr").each(function () {
          ...
          $(document).click(function (e) {
            ...
            if (
              ...
              !$(target).is(".input-dropdown-inner_pr") &&
              !$(target).parents().is(".input-dropdown-inner_pr")
            ) {
              hideList();
              return false;
            }
      ...
      simpleDropdown_qs: function () {
        $(".input-dropdown-inner_qs").each(function () {
          ...
          $(document).click(function (e) {
            ...
            if (
              dd.hasClass("dd-shown") &&
              !$(target).is(".input-dropdown-inner_qs") &&
              !$(target).parents().is(".input-dropdown-inner_qs")
            ) {
              hideList();
              return false;
            }
      ...
      simpleDropdown_qv: function () {
        $(".input-dropdown-inner_qv").each(function () {
          ...
          $(document).click(function (e) {
            ...
            if (
              dd.hasClass("dd-shown") &&
              !$(target).is(".input-dropdown-inner_qv") &&
              !$(target).parents().is(".input-dropdown-inner_qv")
            ) {
              hideList();
              return false;
            }
      ...
      jas_initVideoBackgrounds: function () {
        jQuery("[data-gl-video-bg]").each(function () {
          var youtubeUrl,
            youtubeId,
            $element = jQuery(this);
          $element.data("glVideoBg")
            ? ((youtubeUrl = $element.data("glVideoBg")),
              (youtubeId = geckoShopify.jasExtractYoutubeId(youtubeUrl)),
              youtubeId &&
                ($element.find(".jas_video-bg").remove(),
                geckoShopify.insertYoutubeVideoAsBackground(
                  $element,
                  youtubeId
                )),
      ...
      insertYoutubeVideoAsBackground: function ($element, youtubeId, counter) {
        ...
        var $container = $element
          .prepend(
            '<div class="jas_video-bg hidden-xs"><div class="inner"></div></div>'
          )
          .find(".inner");
          ...
          $.ajax({
            ...
            success: function (responsive) {
              ...
              $.each(data, function (index, element) {
                ...
                html +=
                  '<div class="item pr fl"><a class="db pr oh" href="//instagram.com/p/' +
                  element.code +
                  '" target="' +
                  target +
                  '"><img src="' +
                  ...
                  user_name +
                  '"><div class="jas-wrap-lazy"></div><div class="info pa tc flex ts__03 center-xs middle-xs"><span class="pr cw mgr10"><i class="fa fa-heart-o mr__5"></i>' +
                  element.likes.count +
      ...
      initMegaMenu: function () {
        ...
        var setOffset = function (li) {
          ...
          if (
            ...
            console.log(li);
            var toLeft =
              dropdownOffset.left + dropdownWidth / 2 - screenWidth / 2;
      ...
      ajaxchimp: function () {
        $('form.jas_ajax_mcsp [type="submit"]').bind("click", function (event) {
          ...
          $result.hide(100);
          $button
            .val(gecko_settings.mc_subscribing)
            .html(gecko_settings.mc_subscribing);
        ...
        function register($form, $result, $button) {
          ...
          $.ajax({
            ...
            error: function (err) {
              $button
                .val(gecko_settings.mc_submit)
                .html(gecko_settings.mc_submit);
              var messenger = err
                .replace("0 - ", "")
                .replace("1 - ", "")
                .replace("2 - ", "");
            ...
            success: function (data) {
              ...
              if (data.result != "success") {
                $result
                  .html('<div class="shopify-warning">' + messenger + "</div>")
                  .show(100);
              } else {
                $result
                  .html('<div class="shopify-message">' + messenger + "</div>")
                  .show(100);
              }
            },
          });
          ...
          function () {
            ...
            if (
              ...
              $.magnificPopup.open({
                ...
                items: {
                  src:
                    '<div class="white-popup add-to-cart-popup popup-added_to_cart jas_agree_checkout">' +
                    html +
                    "</div>",
                  type: "inline",
                },
            ...
            success: function (cart) {
              jQuery
                .get("/cart?view=up_ajax", function (data) {
                  ...
                  t_total.html(
                    Shopify.formatMoney(price, gecko_settings.moneyFormat)
                  );
