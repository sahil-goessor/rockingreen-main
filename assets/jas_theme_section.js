...
(function( $ ) {
 ...
 initSearchForm: function() {
  ...
  $('.sf-open').on('touchstart click', function(e) {
    e.preventDefault();
    // show instantly then focus
    HS.show();
    HS.find('input[type="text"]')[0].focus();
  });
...
searchDropdown: function() {
  ...
  var body          = $('body'),
      ...
      inputEl       = searchWrapper.find('input[type="text"]');

  // open/close toggle on touchstart+click
  body.on('touchstart click', '.gecko-search-dropdown>i', function(e) {
    e.preventDefault();
    isOpened() ? closeWidget() : openWidget();
  });

  // close when clicking “close” or outside
  body.on('click', '.gecko-close-search, #jas-header, #jas-content', function(event) {
    if (!$(event.target).is('.gecko-close-search') &&
        !$(event.target).closest('.wrapper-search-dropdown').length) {
      isOpened() && closeWidget();
    }
  });

  function closeWidget() {
    body.removeClass('gecko-search-opened');
    searchWrapper.removeClass('search-overlap').hide();
  }

  function closeByEsc(e) {
    if (e.keyCode === 27) {
      closeWidget();
      body.off('keyup', closeByEsc);
    }
  }

  function openWidget() {
    body.on('keyup', closeByEsc)
        .addClass('gecko-search-opened');
    searchWrapper
      .addClass('search-overlap')
      .show();               // show immediately
    // **direct** focus after show()
    inputEl[0].focus();
  }
